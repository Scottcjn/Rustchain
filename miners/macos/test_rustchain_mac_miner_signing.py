...
class AttestationSignMessageTests(unittest.TestCase):
    ...
    def test_signature_over_sign_message_verifies(self):
        ...
        # A verifier reconstructing the same pipe-string must accept the signature.
        sk.verify_key.verify(msg.encode(), sig)  # raises on mismatch


class ExtractPkcs8Tests(unittest.TestCase):
    def test_rejects_blob_without_ed25519_oid(self):
        module = load_miner_module()
        # Has a 04 20 marker + 32 bytes but NO Ed25519 OID -> must be rejected,
        # not silently treated as a key.
