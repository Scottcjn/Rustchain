...
def _attestation_sign_message(attest_miner_id, wallet, nonce, commitment):
    ...
    Must match the server's verification format exactly:
    ``miner_id|miner|nonce|commitment``.
    """
    return "{}|{}|{}|{}".format(attest_miner_id, wallet, nonce, commitment)


# DER tag for the Ed25519 algorithm OID (1.3.101.112) inside an AlgorithmIdentifier.
...
def _extract_pkcs8_ed25519_seed(der):
    ...
    A v1 Ed25519 PKCS#8 key is:
        SEQUENCE { INTEGER 0, SEQUENCE { OID 1.3.101.112 },
                   OCTET STRING { OCTET STRING(32) seed } }
    so the seed is the final ``04 20 <32 bytes>``. We require the Ed25519 OID to
    be present first, so an unrelated ``04 20`` in arbitrary data can't be
    mistaken for the seed (which would silently load a wrong key).
    """
    if len(der) == 32:
        return der
