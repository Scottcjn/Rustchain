...
| C6: Cross-node reads can be compared for drift | API consistency | Run `python3 tools/node_sync_validator.py` to compare `/health`, `/epoch`, `/api/miners`, and `/api/stats` across live nodes (131, 153, 245), including enrolled miners, miner totals, miner ID set hash, and aggregate stats | Differences stay within expected propagation window and reconcile; same-epoch/same-slot nodes do not disagree on miner state or aggregate stats | Persistent divergence with no reconciliation, or same-epoch/same-slot nodes exposing incompatible miner state |
