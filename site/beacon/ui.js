...
function onObjectHover(hit, event) {
  ...
  if (data.type === 'agent' && data.agentId !== hoveredId) {
    ...
    if (agent) {
      // Get reputation data
      const rep = reputationCache[agent.id] || {};
      const bounties = rep.bounties_completed || 0;
      const contracts = rep.contracts_completed || 0;
      const score = rep.score || agent.score || 0;
      
      // Build tooltip content
      let content = '';
      if (agent.relay) {
        // Relay agent info
        content = `${agent.name} [RELAY]`;
        if (agent.provider) content += ` 
...
Score: ${score}`;
        ...
        // Native agent info  
        content = `${agent.name}`;
        content += ` [${agent.grade || 'N/A'}]`;
        content += ` 
Score: ${score}`;
      }
      
      // Add reputation stats
      if (bounties > 0 || contracts > 0) {
        content += ` 
Bounties: ${bounties} | Contracts: ${contracts}`;
      }
      
      // Add video count if available (from BoTTube)
      if (agent.videos !== undefined) {
        content += ` 
Videos: ${agent.videos}`;
      }
...
/g, '<br>');
    ...
    if (city) {
      const agentsInCity = AGENTS.filter(a => a.city === city.id).length;
      tooltip.innerHTML = `<strong>${city.name}</strong><br>Population: ${city.population}<br>Agents: ${agentsInCity}`;
      tooltip.classList.add('visible');
    }
  }
