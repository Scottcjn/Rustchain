...
class TipBot:
    ...
    def process_tip(
        ...
        # guarded by _balances_lock to prevent TOCTOU race conditions.
        with self._balances_lock:
            # Re-read sender balance under lock to prevent double-spend
            sender_bal_locked = Decimal(str(balances.get(sender, 0.0)))
            if sender_bal_locked < dec_amount:
                return TipResult(
                    success=False, error_message="Insufficient balance for tip"
                )

            new_sender_bal = (sender_bal_locked - dec_amount).quantize(
                Decimal("0.00000001"), rounding=ROUND_HALF_UP
            )
            new_recipient_bal = (
...
class RewardCalculator:
    ...
    def claim_reward(
        ...
        # Check frequency cap under lock to prevent TOCTOU race conditions
