...
_REGISTRY_ADMIN_KEY = os.environ.get("REGISTRY_ADMIN_KEY", "")

# Wallet address validation pattern (0x or RTC prefix, min 10 chars)
WALLET_PATTERN = re.compile(r"^(0x[0-9a-fA-F]{10,}|RTC[0-9a-zA-Z]{10,})$")
...
def _require_admin():
    """Check admin authentication for mutating endpoints."""
    if not _REGISTRY_ADMIN_KEY:
        return jsonify({"error": "unauthorized",
                        "message": "REGISTRY_ADMIN_KEY not configured"}), 401
    admin_key = request.headers.get("X-Admin-Key", "") or request.headers.get("X-API-Key", "")
    if not hmac.compare_digest(admin_key, _REGISTRY_ADMIN_KEY):
...
def register():
    ...
    if not _validate_wallet(rtc_wallet):
        flash('Error: Invalid wallet address format. Must start with 0x or RTC and be at least 10 characters.')
    ...
    try:
        with sqlite3.connect(DB_PATH) as conn:
            conn.execute(
                'INSERT INTO contributors (github_username, contributor_type, rtc_wallet, contribution_history, status) VALUES (?, ?, ?, ?, ?)',
                (github_username, contributor_type, rtc_wallet, contribution_history, 'pending')
            )
            conn.commit()
...
@app.route('/api/contributors/<username>/approve', methods=['POST'])
def approve_contributor(username):
    """Admin-only: approve a pending contributor registration."""
    auth_error = _require_admin()
    if auth_error:
        return auth_error
    with sqlite3.connect(DB_PATH) as conn:
        result = conn.execute(
            'UPDATE contributors SET status = ? WHERE github_username = ? AND status = ?',
            ('approved', username, 'pending')
        )
        conn.commit()
        if result.rowcount == 0:
            return jsonify({"error": "not found or already approved"}), 404
    return jsonify({"ok": True, "username": username, "status": "approved"})
