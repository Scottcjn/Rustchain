...
def _require_admin():
    """Check admin authentication for fingerprint submission."""
    if not _SOPHIA_ADMIN_KEY:
        return jsonify({"error": "unauthorized",
                        "message": "SOPHIA_ADMIN_KEY not configured"}), 401
    admin_key = request.headers.get("X-Admin-Key", "") or request.headers.get("X-API-Key", "")
    if not hmac.compare_digest(admin_key, _SOPHIA_ADMIN_KEY):
