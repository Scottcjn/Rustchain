...
# Trusted peer IPs - configurable via environment, no auth bypass
# SECURITY: Trusted peers still authenticate, but get wider timestamp window
_TRUSTED_PEER_IPS = set(
    ip.strip()
    for ip in os.environ.get("RC_TRUSTED_PEERS", "").split(",")
    if ip.strip()
)
