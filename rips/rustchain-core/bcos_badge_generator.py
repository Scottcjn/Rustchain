...
_BCOS_ADMIN_KEY = os.environ.get("BCOS_ADMIN_KEY", "")


def _require_admin():
    """Require admin key for badge generation."""
    if not _BCOS_ADMIN_KEY:
        return jsonify({
            'success': False,
            'error': 'BCOS_ADMIN_KEY not configured — set env var to enable badge generation'
        }), 503
    admin_key = request.headers.get("X-Admin-Key", "") or request.headers.get("X-API-Key", "")
    if not hmac.compare_digest(admin_key, _BCOS_ADMIN_KEY):
...
def generate_badge():
    """Generate a BCOS badge (requires admin authentication)."""
