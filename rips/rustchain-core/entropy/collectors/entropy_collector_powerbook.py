#!/usr/bin/env python3
"""
RustChain Entropy Collector - PowerBook Edition (macOS/Tiger)
==============================================================

"Every vintage computer has historical potential"

Collects deep hardware entropy from PowerBook G4 systems for validator fingerprinting.
Makes emulation economically irrational.

Usage: python entropy_collector_powerbook.py
       OR
       python2 entropy_collector_powerbook.py (for Tiger compatibility)
"""

from __future__ import print_function  # Python 2 compatibility

import hashlib
import json
import os
import platform
import socket
import subprocess
import sys
import time

# Python 2/3 compatibility
if sys.version_info[0] >= 3:
    from typing import Dict, List, Optional

# =============================================================================
# Helper Functions
# =============================================================================

def run_cmd(cmd, default=""):
    """Safely run a command"""
    try:
        proc = subprocess.Popen(
            cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
        stdout, stderr = proc.communicate()
        if sys.version_info[0] >= 3:
            return stdout.decode('utf-8', errors='ignore').strip()
        return stdout.strip()
    except:
        return default


def read_file(path, default=""):
    """Safely read a file"""
    try:
        with open(path, 'r') as f:
            return f.read().strip()
    except:
        return default


# =============================================================================
# Entropy Collection
# =============================================================================

def collect_cpu_info():
    """Collect CPU information"""
    info = {
        "model": "",
        "vendor": "",
        "cores": 1,
        "frequency_mhz": 0,
        "l1_cache": 0,
        "l2_cache": 0,
    }

    # Try sysctl for macOS
    info["model"] = run_cmd("sysctl -n machdep.cpu.brand_string") or run_cmd("sysctl -n hw.model")
    info["cores"] = int(run_cmd("sysctl -n hw.ncpu") or "1")

    # CPU frequency
    freq = run_cmd("sysctl -n hw.cpufrequency")
    if freq:
        info["frequency_mhz"] = int(freq) // 1000000

    # Cache
    l1 = run_cmd("sysctl -n hw.l1dcachesize")
    if l1:
        info["l1_cache"] = int(l1) // 1024

    l2 = run_cmd("sysctl -n hw.l2cachesize")
    if l2:
        info["l2_cache"] = int(l2) // 1024

    return info


def collect_memory_info():
    """Collect memory information"""
    info = {"total_mb": 0, "type": "Unknown"}

    mem = run_cmd("sysctl -n hw.memsize")
    if mem:
        info["total_mb"] = int(mem) // (1024 * 1024)

    # Try system_profiler for RAM type
    profiler = run_cmd("system_profiler SPMemoryDataType 2>/dev/null | grep 'Type:' | head -1")
    if profiler and ":" in profiler:
        info["type"] = profiler.split(":")[-1].strip()

    return info


def collect_disk_info():
    """Collect disk information"""
    info = {
        "model": "",
        "serial": "",
        "size_gb": 0,
    }

    # Try system_profiler
    profiler = run_cmd("system_profiler SPSerialATADataType SPParallelATADataType 2>/dev/null")
    for line in profiler.split("\n"):
        if "Model:" in line and not info["model"]:
            info["model"] = line.split(":")[-1].strip()
        elif "Serial Number:" in line and not info["serial"]:
            info["serial"] = line.split(":")[-1].strip()
        elif "Capacity:" in line and not info["size_gb"]:
            try:
                cap_str = line.split(":")[-1].strip()
                # Parse "80.03 GB" style
                info["size_gb"] = float(cap_str.split()[0])
            except:
                pass

    return info


def collect_network_info():
    """Collect MAC addresses"""
    macs = []

    # Try ifconfig
    ifconfig = run_cmd("ifconfig -a")
    for line in ifconfig.split("\n"):
        if "ether" in line.lower():
            parts = line.split()
            for i, p in enumerate(parts):
                if p.lower() == "ether" and i + 1 < len(parts):
                    mac = parts[i + 1]
                    if mac and mac != "00:00:00:00:00:00":
                        macs.append(mac)

    return macs


def collect_system_info():
    """Collect system identification"""
    info = {
        "hostname": socket.gethostname(),
        "serial_number": "",
        "model_identifier": "",
        "boot_rom_version": "",
        "smc_version": "",
    }

    # Serial number from ioreg
    serial = run_cmd("ioreg -l 2>/dev/null | grep IOPlatformSerialNumber | head -1")
    if serial and '"' in serial:
        try:
            info["serial_number"] = serial.split('"')[-2]
        except:
            pass

    # Model from system_profiler
    profiler = run_cmd("system_profiler SPHardwareDataType 2>/dev/null")
    for line in profiler.split("\n"):
        if "Model Identifier:" in line:
            info["model_identifier"] = line.split(":")[-1].strip()
        elif "Boot ROM Version:" in line:
            info["boot_rom_version"] = line.split(":")[-1].strip()
        elif "SMC Version" in line:
            info["smc_version"] = line.split(":")[-1].strip()

    return info


def collect_gpu_info():
    """Collect GPU information"""
    info = {
        "model": "",
        "vendor": "",
        "vram_mb": 0,
    }

    profiler = run_cmd("system_profiler SPDisplaysDataType 2>/dev/null")
    for line in profiler.split("\n"):
        if "Chipset Model:" in line:
            info["model"] = line.split(":")[-1].strip()
        elif "Vendor:" in line and not info["vendor"]:
            info["vendor"] = line.split(":")[-1].strip()
        elif "VRAM" in line:
            try:
                vram_str = line.split(":")[-1].strip()
                info["vram_mb"] = int(vram_str.split()[0])
            except:
                pass

    return info


def collect_openfirmware_info():
    """Collect OpenFirmware/NVRAM info"""
    info = {
        "nvram_sample": "",
        "boot_args": "",
    }

    # NVRAM sample
    try:
        with open("/dev/nvram", "rb") as f:
            data = f.read(32)
            if sys.version_info[0] >= 3:
                info["nvram_sample"] = data.hex()
            else:
                info["nvram_sample"] = data.encode('hex')
    except:
        pass

    # Boot args
    info["boot_args"] = run_cmd("nvram boot-args 2>/dev/null")

    return info


def collect_timing_entropy():
    """Collect timing-based entropy"""
    samples = []

    for i in range(32):
        start = time.time()
        # Small computation
        _ = sum(range(i * 10 + 1))
        end = time.time()
        samples.append(int((end - start) * 1000000000))  # nanoseconds

    return samples


def collect_os_info():
    """Collect OS information"""
    info = {
        "version": "",
        "darwin_version": "",
        "kernel": "",
    }

    info["version"] = run_cmd("sw_vers -productVersion")
    info["darwin_version"] = run_cmd("uname -r")
    info["kernel"] = platform.release()

    return info


# =============================================================================
# Main Collection
# =============================================================================

def collect_all_entropy():
    """Collect all entropy sources"""
    print("Collecting PowerBook hardware entropy...")

    print("  [1/9] CPU info...")
    cpu = collect_cpu_info()

    print("  [2/9] Memory info...")
    mem = collect_memory_info()

    print("  [3/9] Disk info...")
    disk = collect_disk_info()

    print("  [4/9] Network info...")
    macs = collect_network_info()

    print("  [5/9] System info...")
    system = collect_system_info()

    print("  [6/9] GPU info...")
    gpu = collect_gpu_info()

    print("  [7/9] OpenFirmware info...")
    of = collect_openfirmware_info()

    print("  [8/9] Timing entropy...")
    timing = collect_timing_entropy()

    print("  [9/9] OS info...")
    os_info = collect_os_info()

    return {
        "cpu": cpu,
        "memory": mem,
        "disk": disk,
        "mac_addresses": macs,
        "system": system,
        "gpu": gpu,
        "openfirmware": of,
        "timing_samples": timing,
        "os": os_info,
    }


def generate_entropy_proof(data):
    """Generate entropy proof from collected data"""
    print("\nGenerating entropy proof...")

    # Combine data
    entropy_json = json.dumps(data, sort_keys=True)
    if sys.version_info[0] >= 3:
        entropy_bytes = entropy_json.encode('utf-8')
    else:
        entropy_bytes = entropy_json

    # SHA256
    sha256_hash = hashlib.sha256(entropy_bytes).hexdigest()

    # Fingerprint
    fingerprint_data = (
        sha256_hash +
        data["system"]["serial_number"] +
        "".join(data["mac_addresses"]) +
        data["disk"]["serial"]
    )
    if sys.version_info[0] >= 3:
        fingerprint_bytes = fingerprint_data.encode('utf-8')
    else:
        fingerprint_bytes = fingerprint_data
    fingerprint = hashlib.sha256(fingerprint_bytes).hexdigest()

    # Determine tier (PowerBook G4 is vintage!)
    arch = platform.machine()
    if arch in ["Power Macintosh", "ppc", "ppc64"]:
        tier = "vintage"
        multiplier = 2.5
    else:
        tier = "modern"
        multiplier = 1.0

    # Count sources
    sources = 0
    if data["cpu"]["model"]: sources += 1
    if data["memory"]["total_mb"]: sources += 1
    if data["disk"]["serial"]: sources += 1
    if data["mac_addresses"]: sources += 1
    if data["system"]["serial_number"]: sources += 1
    if data["gpu"]["model"]: sources += 1
    if data["openfirmware"]["nvram_sample"]: sources += 1
    if data["timing_samples"]: sources += 1
    if data["os"]["version"]: sources += 1

    timestamp = int(time.time())
    signature = "POWERBOOK-PPC-ENTROPY-%s-%d-D%d" % (fingerprint[:16], timestamp, sources)

    return {
        "sha256_hash": sha256_hash,
        "deep_fingerprint": fingerprint,
        "signature": signature,
        "tier": tier,
        "multiplier": multiplier,
        "entropy_sources": sources,
        "timestamp": timestamp,
        "hardware_verified": True,
    }


def write_output(data, proof):
    """Write JSON output"""
    output = {
        "rustchain_entropy": {
            "version": 1,
            "platform": "macos_powerbook",
            "collector": "entropy_collector_powerbook.py",
            "timestamp": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
        },
        "proof_of_antiquity": {
            "philosophy": "Every vintage computer has historical potential",
            "consensus": "NOT Proof of Work - This is PROOF OF ANTIQUITY",
            "hardware_verified": proof["hardware_verified"],
            "tier": proof["tier"],
            "multiplier": proof["multiplier"],
        },
        "entropy_proof": {
            "sha256_hash": proof["sha256_hash"],
            "deep_fingerprint": proof["deep_fingerprint"],
            "signature": proof["signature"],
            "entropy_sources": proof["entropy_sources"],
            "sources": [
                "cpu_identification",
                "memory_configuration",
                "disk_serial",
                "mac_addresses",
                "system_serial",
                "gpu_identification",
                "openfirmware_nvram",
                "timing_entropy",
                "os_fingerprint",
            ],
        },
        "hardware_profile": {
            "hostname": data["system"]["hostname"],
            "serial_number": data["system"]["serial_number"],
            "model_identifier": data["system"]["model_identifier"],
            "boot_rom": data["system"]["boot_rom_version"],
            "cpu": data["cpu"],
            "memory": data["memory"],
            "disk": data["disk"],
            "gpu": data["gpu"],
            "mac_addresses": data["mac_addresses"],
            "openfirmware": data["openfirmware"],
            "os": data["os"],
        },
    }

    hostname = data["system"]["hostname"].replace(" ", "_").replace(".", "_")
    filename = "entropy_powerbook_%s.json" % hostname
    with open(filename, "w") as f:
        json.dump(output, f, indent=2)

    print("\nEntropy profile written to: %s" % filename)
    return output


def main():
    print("")
    print("=" * 70)
    print("   RUSTCHAIN ENTROPY COLLECTOR - POWERBOOK G4 EDITION")
    print("")
    print('   "Every vintage computer has historical potential"')
    print("")
    print("   Collecting hardware entropy to prove YOU ARE NOT AN EMULATOR")
    print("=" * 70)
    print("")

    # Detect architecture
    arch = platform.machine()
    print("  Architecture: %s" % arch)
    if arch in ["Power Macintosh", "ppc", "ppc64"]:
        print("  PowerPC detected - VINTAGE TIER!")
    print("")

    # Collect
    data = collect_all_entropy()

    # Generate proof
    proof = generate_entropy_proof(data)

    # Write output
    output = write_output(data, proof)

    # Print summary
    print("")
    print("=" * 70)
    print("                    HARDWARE PROFILE SUMMARY")
    print("=" * 70)
    print("  Hostname: %s" % data["system"]["hostname"])
    print("  Serial: %s" % data["system"]["serial_number"])
    print("  Model: %s" % data["system"]["model_identifier"])
    print("  OS: Mac OS X %s" % data["os"]["version"])
    print("  CPU: %s" % data["cpu"]["model"])
    print("  RAM: %d MB" % data["memory"]["total_mb"])
    print("  Disk: %s (%s)" % (data["disk"]["model"], data["disk"]["serial"]))
    print("  GPU: %s" % data["gpu"]["model"])
    if data["mac_addresses"]:
        print("  MACs: %s" % ", ".join(data["mac_addresses"][:3]))
    print("")
    print("=" * 70)
    print("                    ENTROPY PROOF")
    print("=" * 70)
    print("  Signature: %s" % proof["signature"])
    print("  SHA256: %s..." % proof["sha256_hash"][:32])
    print("  Fingerprint: %s..." % proof["deep_fingerprint"][:32])
    print("  Entropy Sources: %d" % proof["entropy_sources"])
    print("  Hardware Tier: %s (%sx)" % (proof["tier"].upper(), proof["multiplier"]))
    print("")
    print("=" * 70)
    print("                    ENTROPY COLLECTION COMPLETE")
    print("")
    print("   This fingerprint proves your hardware is REAL")
    print("   Emulation is economically irrational.")
    print("=" * 70)
    print("")

    # Output JSON for remote capture
    print("=== BEGIN JSON OUTPUT ===")
    print(json.dumps(output, indent=2))
    print("=== END JSON OUTPUT ===")

    return proof


if __name__ == "__main__":
    main()
