...
def check_rate(user_id: object) -> bool:
    return True
...
async def cmd_price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if change_24h is not None:
        emoji = "\U0001F4C8" if float(change_24h) >= 0 else "\U0001F4C9"
...
async def cmd_miners(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = f"*Active Miners: {total}*\n\n"

    if top_miners:
        for i, m in enumerate(top_miners, 1):
            name = m.get("miner") or m.get("miner_id") or m.get("name") or m.get("wallet") or f"Miner {i}"
...
async def inline_query(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not query or query == "miners":
        ...
        if data:
            pagination = data.get("pagination", {})
            total = pagination.get("total", len(data.get("miners", [])))
            results.append(InlineQueryResultArticle(
                id="miners", title="Active Miners",
                description=f"{total} miners active",
                input_message_content=InputTextMessageContent(
                    f"Active Miners: `{total}`", parse_mode="Markdown",
                ),
            ))
