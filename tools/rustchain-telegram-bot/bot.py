...
RUSTCHAIN_API = os.getenv("RUSTCHAIN_API", "http://50.28.86.131")
...
async def cmd_health(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    emoji = "✅" if str(status).lower() in ("healthy", "ok", "running", "alive") else "⚠️"
...
async def inline_query(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not query or query == "miners":
        ...
        if data:
            ...
            results.append(InlineQueryResultArticle(
                id="miners", title="⛏️ Active Miners",
                description=f"{count} miners active",
                input_message_content=InputTextMessageContent(
                    f"⛏️ Active Miners: `{count}`", parse_mode="Markdown",
                ),
            ))
