#!/usr/bin/env python3
"""RustChain Calendar view of epoch participation."""
...
def main():
    ...
    print(f"Calendar view of epoch participation")
    print(f"  {h.get('status','?')} v{h.get('version','?')} | epoch {e.get('epoch', e.get('current_epoch','?'))} slot {e.get('slot','?')} | {len(ml)} miners | {hms}ms")
