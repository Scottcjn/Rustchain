#!/usr/bin/env python3
"""RustChain Track power consumption estimates."""
import json, urllib.request, ssl, os, time
NODE = os.environ.get("RUSTCHAIN_NODE", "https://rustchain.org")
def api(p):
    ctx = ssl.create_default_context(); ctx.check_hostname = False; ctx.verify_mode = ssl.CERT_NONE
    try:
        s = time.time()
        r = urllib.request.urlopen(f"{NODE}{p}", timeout=10, context=ctx)
        return json.loads(r.read()), round((time.time()-s)*1000, 1)
    except: return {}, 0
def main():
    h, hms = api("/health")
    e, ems = api("/epoch")
    t, tms = api("/headers/tip")
    print(f"Track power consumption estimates")
    print(f"  Node: {h.get('status','?')} v{h.get('version','?')} ({hms}ms)")
    print(f"  Epoch: {e.get('epoch', e.get('current_epoch','?'))} Slot: {e.get('slot','?')} ({ems}ms)")
    print(f"  Height: {t.get('height', t.get('slot','?'))} ({tms}ms)")
if __name__ == "__main__":
    main()
