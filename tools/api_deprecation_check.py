#!/usr/bin/env python3
"""RustChain Check for deprecated endpoints."""
...
def main():
    ...
    print(f"Check for deprecated endpoints")
    ...
    print(f"  Network: {len(ml)} miners | {e.get('epoch_pot', e.get('reward_pot','?'))} RTC pot | {f.get('fee_pool', f.get('balance','?'))} RTC fees")
