...
**La primera blockchain que recompensa hardware antiguo por ser viejo, no rápido.**

*Tu PowerPC G4 gana más que un Threadripper moderno. Ese es el punto.*

[Sitio Web](https://rustchain.org) • [Explorador en Vivo](https://rustchain.org/explorer) • [Intercambiar wRTC](https://raydium.io/swap/?inputMint=sol&outputMint=12TAdKXxcGf6oCv4rqDz2NkgxjyHq6HQKoxKZYGf5i4X) • [DexScreener](https://dexscreener.com/solana/8CF2Q8nSCxRacDShbtF86XTSrYjueBMKmfdR3MLdnYzb) • [Guía Rápida wRTC](docs/wrtc.md) • [Tutorial wRTC](docs/WRTC_ONBOARDING_TUTORIAL.md) • [Referencia Grokipedia](https://grokipedia.com/search?q=RustChain) • [Whitepaper](docs/RustChain_Whitepaper_Flameholder_v0.97-1.pdf) • [Inicio Rápido](#-inicio-rápido) • [Cómo Funciona](#-cómo-funciona-proof-of-antiquity)
...
| **Puente RTC ↔ wRTC** | [Puente BoTTube](https://bottube.ai/bridge) |
| **Guía de Inicio Rápido** | [Inicio Rápido wRTC](docs/wrtc.md) |
| **Tutorial de Incorporación** | [Guía de Seguridad del Puente wRTC](docs/WRTC_ONBOARDING_TUTORIAL.md) |
...
Cada contribución gana tokens RTC. Correcciones de errores, características, documentación, auditorías de seguridad — todo es pagado.

| Nivel | Recompensa | Ejemplos |
|------|--------|----------|
| Micro | 1-10 RTC | Corrección de errores tipográficos, documentación pequeña, prueba simple |
...
3. Fork, corrige, PR — recibe pago en RTC
4. Ver [CONTRIBUTING.md](CONTRIBUTING.md) para detalles completos

**1 RTC = $0.10 USD** | `pip install clawrtc` para comenzar a minar

---

## Carteras de Agentes + Pagos x402

Los agentes de RustChain ahora pueden poseer **carteras Coinbase Base** y realizar pagos máquina-a-máquina usando el **protocolo x402** (HTTP 402 Payment Required):

| Recurso | Enlace |
|----------|------|
| **Documentación de Carteras de Agentes** | [rustchain.org/wallets.html](https://rustchain.org/wallets.html) |
...
**Endpoints de API Premium x402** están activos (actualmente gratis mientras se prueba el flujo):
...
| **PSE Hardware Entropy** | [![DOI](https://zenodo.org/badge/DOI/10.5281/zenodo.18623922.svg)](https://doi.org/10.5281/zenodo.18623922) | Entropía POWER8 mftb para divergencia conductual |
| **Neuromorphic Prompt Translation** | [![DOI](https://zenodo.org/badge/DOI/10.5281/zenodo.18623594.svg)](https://doi.org/10.5281/zenodo.18623594) | Prompting emocional para ganancias del 20% en difusión de video |
...
| Más nuevo = Mejor | Más viejo = Mejor |
| Consumo de energía desperdiciado | Preserva la historia de la computación |
| Carrera hacia el fondo | Recompensa la preservación digital |

**Principio Central**: El hardware antiguo auténtico que ha sobrevivido décadas merece reconocimiento. RustChain invierte la minería.
...
**Instalar con una cartera específica:**
```bash
curl -sSL https://raw.githubusercontent.com/Scottcjn/Rustchain/main/install-miner.sh | bash -s -- --wallet mi-cartera-minero
```
...
- **El instalador falla con errores de permisos**: vuelve a ejecutar usando una cuenta con acceso de escritura a `~/.local` y evita ejecutar dentro de site-packages globales de Python del sistema.
- **Errores de versión de Python** (`SyntaxError` / `ModuleNotFoundError`): instala con Python 3.10+ y configura `python3` a ese intérprete.
  ```bash
  python3 --version
  curl -sSL https://raw.githubusercontent.com/Scottcjn/Rustchain/main/install-miner.sh | bash
  ```
- **Errores de certificado HTTPS en `curl`**: esto puede suceder con entornos de cliente que no son navegadores; verifica la conectividad primero con `curl -I https://rustchain.org` antes de verificaciones de cartera.
- **El minero sale inmediatamente**: verifica que la cartera existe y el servicio está ejecutándose (`systemctl --user status rustchain-miner` o `launchctl list | grep rustchain`)

Si un problema persiste, incluye registros y detalles del SO en un nuevo issue o comentario de bounty con la salida exacta del error y tu resultado de `install-miner.sh --dry-run`.

### Después de la Instalación

**Verifica el saldo de tu cartera:**
```bash
# Nota: Usando flags -sk porque el nodo puede usar un certificado SSL autofirmado
curl -sk "https://rustchain.org/wallet/balance?miner_id=NOMBRE_DE_TU_CARTERA"
```

**Lista mineros activos:**
```bash
...
**Obtén la época actual:**
```bash
curl -sk https://rustchain.org/epoch
```

**Gestiona el servicio del minero:**
...
journalctl --user -u rustchain-miner -f    # Ver registros
```
...
tail -f ~/.rustchain/miner.log             # Ver registros
```
...
cd Rustchain
bash install-miner.sh --wallet NOMBRE_DE_TU_CARTERA
# Opcional: previsualizar acciones sin cambiar tu sistema
bash install-miner.sh --dry-run --wallet NOMBRE_DE_TU_CARTERA
```
...
| **Cazador de Agentes IA** | 200 RTC | [Bounty de Agente #34](https://github.com/Scottcjn/rustchain-bounties/issues/34) |
...
| Hardware | Era | Multiplicador | Ganancias de Ejemplo |
|----------|-----|------------|------------------|
...
│ 6. Anti-Emulation Checks           ← Detectar VMs/emuladores   │
└─────────────────────────────────────────────────────────────┘
```

**Por qué importa**: Una VM SheepShaver pretendiendo ser una Mac G4 fallará estas verificaciones. El silicio antiguo real tiene patrones de envejecimiento únicos que no pueden falsificarse.
...
- Las recompensas se dividen equitativamente entre todos los votantes, luego se multiplican por antigüedad
- Sin ventaja por ejecutar múltiples hilos o CPUs más rápidas
...
Distribución: División equitativa × multiplicador de antigüedad
```
...
curl -sk "https://rustchain.org/wallet/balance?miner_id=TU_CARTERA"
...
| 🏛️ **Pantheon Pioneer** | Primeros 100 mineros | Limitado |

## 🔒 Modelo de Seguridad

### Detección Anti-VM
Las VMs son detectadas y reciben **una mil millonésima** de las recompensas normales:
```
Mac G4 Real:    multiplicador 2.5×  = 0.30 RTC/época
G4 Emulado:     0.0000000025×       = 0.0000000003 RTC/época
```

### Vinculación de Hardware
Cada huella digital de hardware está vinculada a una cartera. Previene:
- Múltiples carteras en el mismo hardware
- Suplantación de hardware
- Ataques Sybil
...
Lee la especificación borrador:
...
| **BoTTube** | [bottube.ai](https://bottube.ai) - Plataforma de video IA |
| **Moltbook** | [moltbook.com](https://moltbook.com) - Red social IA |
...
**Un año de desarrollo, hardware antiguo real, facturas de electricidad y un laboratorio dedicado se invirtieron en esto.**
...
Licencia MIT - Libre para usar, pero por favor mantén el aviso de copyright y atribución.
...
![Estado de Minería RustChain](https://img.shields.io/endpoint?url=https://rustchain.org/api/badge/frozen-factorio-ryan&style=flat-square)<!-- rustchain-mining-badge-end -->
...
Esperado: todas las 6 verificaciones de huella digital de hardware se ejecutan en ARM64 nativo sin errores de respaldo de arquitectura.
