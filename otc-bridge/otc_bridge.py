...
def rtc_address_from_public_key(public_key_hex):
    pub_bytes = safe_fromhex(public_key_hex)
...
def require_wallet_auth(data, action, order_id, wallet, bound_fields=None):
    ...
    try:
        ...
        sig_bytes = safe_fromhex(signature)
        if pub_bytes is None or sig_bytes is None:
            return "wallet_auth_invalid_encoding"
        
        verify_key = Ed25519PublicKey.from_public_bytes(pub_bytes)
        verify_key.verify(
            sig_bytes,
        ...
        return "wallet_auth_internal_error"
...
def generate_htlc_secret():
    ...
    secret_bytes = safe_fromhex(secret)
    # secret is generated by token_hex, so it's always valid.
    hash_val = hashlib.sha256(secret_bytes or b"").hexdigest()
