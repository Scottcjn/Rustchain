...
def init_replay_defense_schema():
    ...
    with sqlite3.connect(get_db_path()) as conn:
        ...
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_entropy_collisions_hash ON entropy_collisions(entropy_profile_hash)"
        )
...
def check_fingerprint_replay(
    fingerprint_hash: str, nonce: str, wallet_address: str, miner_id: str
...
def check_entropy_collision(
    entropy_profile_hash: str, wallet_address: str, miner_id: str
) -> Tuple[bool, str, Optional[Dict]]:
    """
    ...
    with sqlite3.connect(get_db_path()) as conn:
        ...
        if collisions:
            ...
            for coll_wallet, _coll_miner, _ in collisions:
                c.execute(
                    """
...
def check_fingerprint_rate_limit(hardware_id: str, wallet_address: str) -> Tuple[bool, str, Optional[Dict]]:
    """
...
def record_fingerprint_submission(
    ...
    checks_hash = hashlib.sha256(json.dumps(fingerprint.get("checks", {}), sort_keys=True).encode()).hexdigest()
    ...
    with sqlite3.connect(get_db_path()) as conn:
        ...
        """,
            (
                ...
                nonce,
                now,
                entropy_profile_hash,
                checks_hash,
                1 if attestation_valid else 0,
            ),
        )
...
def detect_fingerprint_anomalies(miner_id: str, wallet_address: str, fingerprint_hash: str) -> Tuple[bool, List[Dict]]:
    """
    ...
    with sqlite3.connect(get_db_path()) as conn:
        ...
        for prev_hash, _prev_seq, prev_time, prev_wallet in history[1:]:
...
def get_replay_defense_report(
    wallet_address: Optional[str] = None, miner_id: Optional[str] = None, hours: int = 24
) -> Dict:
    """
