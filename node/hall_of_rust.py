...
def induct_machine():
    ...
    try:
        ...
        if existing:
            ...
            conn.close()
            return jsonify(
                {"inducted": False, "message": "Already in Hall of Rust", "attestation_count": existing[1] + 1}
            )
        ...
        c.execute("UPDATE hall_of_rust SET rust_score = ? WHERE fingerprint_hash = ?", (rust_score, fingerprint_hash))
...
def rust_leaderboard():
    ...
    try:
        ...
        return jsonify(
            {"leaderboard": leaderboard, "total_machines": len(leaderboard), "generated_at": int(time.time())}
        )
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def hall_stats():
    ...
    try:
        ...
        c.execute(
            """SELECT SUM(total_attestations) FROM hall_of_rust WHERE device_arch NOT IN ('unknown', 'default')"""
        )
...
def random_fact():
    ...
    return jsonify({"fact": random.choice(VINTAGE_FACTS), "generated_at": int(time.time())})
...
def fleet_breakdown():
    ...
    try:
        ...
        conn.close()
        return jsonify(
            {"breakdown": breakdown, "total_architectures": len(breakdown), "generated_at": int(time.time())}
        )
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def hall_timeline():
    ...
    try:
        ...
        for row in c.fetchall():
            timeline.append(
                {"date": row[0], "machines_joined": row[1], "architectures": row[2].split(",") if row[2] else []}
            )

        conn.close()
        return jsonify({"timeline": timeline, "generated_at": int(time.time())})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
