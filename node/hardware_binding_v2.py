...
def bind_hardware_v2(
    serial: str, wallet: str, arch: str, cores: int, fingerprint: dict, macs: list = None
) -> Tuple[bool, str, dict]:
    """
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        c.execute(
            "SELECT bound_wallet, entropy_profile, macs_seen, attestation_count FROM hardware_bindings_v2 WHERE serial_hash = ?",
            (serial_hash,),
        )
        row = c.fetchone()
        ...
        if row is None:
            ...
            return True, "new_binding", {"serial_hash": serial_hash[:16], "wallet": wallet[:20], "status": "bound"}
        ...
        return (
            True,
            "authorized",
            {"serial_hash": serial_hash[:16], "similarity": f"{similarity:.1%}", "attestations": attest_count + 1},
        )
