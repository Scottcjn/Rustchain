...
class BlockchainIntegration:
    ...
    def __init__(self, node_url: str = "https://rustchain.org:8085", db_path: str = "db/rustchain_miners.db"):
    ...
    def process_new_block(self, block_data: Dict) -> Dict:
        ...
        results = {"new_miners": 0, "updated_miners": 0, "badges_awarded": 0, "errors": []}
    ...
    def _process_miner(self, miner_data: Dict, block_height: int) -> Tuple[bool, str]:
        ...
        try:
            ...
            if not existing:
                ...
                self.db.update_mining_stats(miner_data["wallet"], block_height, miner_data["reward"])
    ...
    def _parse_hardware_string(self, hardware_str: str) -> Dict:
        ...
        hardware_info = {"model": hardware_str, "generation": "Unknown", "age_years": 5, "cpu_family": 0}

        lower = hardware_str.lower()

        # PowerPC parsing
        if "powerpc g4" in lower or "g4" in lower:
            hardware_info.update({"generation": "PowerPC G4", "age_years": 22, "cpu_family": 74})
        elif "powerpc g3" in lower:
            hardware_info.update({"generation": "PowerPC G3", "age_years": 26, "cpu_family": 74})
        # Intel parsing
        elif "486" in lower:
            hardware_info.update({"generation": "Intel 486", "age_years": 35, "cpu_family": 4})
        elif "386" in lower:
            hardware_info.update({"generation": "Intel 386", "age_years": 38, "cpu_family": 3})
        elif "pentium" in lower:
            if "ii" in lower or "2" in lower:
                hardware_info.update({"generation": "Pentium II", "age_years": 27, "cpu_family": 6})
            elif "iii" in lower or "3" in lower:
                hardware_info.update({"generation": "Pentium III", "age_years": 25, "cpu_family": 6})
        # Modern CPUs
        elif "xeon" in lower:
            if "scalable" in lower:
                hardware_info.update({"generation": "Xeon Scalable", "age_years": 3, "cpu_family": 6})
            else:
                hardware_info.update({"generation": "Xeon", "age_years": 5, "cpu_family": 6})
        elif "ryzen" in lower:
            hardware_info.update({"generation": "AMD Ryzen", "age_years": 5, "cpu_family": 23})
            ...
            for badge_type in eligible_badges:
                if badge_type not in existing_badges:
                    badge_id = self.db.award_badge(
                        wallet, badge_type, self.badge_generator.BADGE_TYPES[badge_type]["tier"], block_height
                    )
                    ...
                    if badge_id:
                        ...
                        metadata = self.badge_generator.generate_badge_metadata(badge_type, miner_stats, block_height)
    ...
    def sync_with_blockchain(self) -> Dict:
        ...
        results = {"blocks_processed": 0, "new_miners": 0, "badges_awarded": 0, "errors": []}
    ...
    def generate_miner_certificate(self, wallet: str) -> Optional[Dict]:
        ...
        certificate = {
            ...
            "badges": [
                {"type": badge["badge_type"], "tier": badge["badge_tier"], "earned": badge["earned_timestamp"]}
