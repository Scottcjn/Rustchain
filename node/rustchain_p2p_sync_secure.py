...
class P2PAuthManager:
    ...
    def verify_peer_signature(self, signature: str, message: str, timestamp: str) -> bool:
        ...
        for key in [self._current_key, self._previous_key]:
            ...
            expected_sig = hmac.new(key.encode(), message_bytes, hashlib.sha256).hexdigest()
    ...
    def generate_signature(self, message: str) -> tuple:
        ...
        signature = hmac.new(self._current_key.encode(), message_bytes, hashlib.sha256).hexdigest()
...
class RateLimiter:
    ...
    def __init__(self):
        ...
        self.limits = {"/p2p/blocks": 10, "/p2p/transactions": 100, "/p2p/ping": 60, "default": 50}
    ...
    def check_rate_limit(self, peer_url: str, endpoint: str) -> bool:
        ...
        with self.lock:
            ...
            self.requests[peer_url] = [(ts, ep) for ts, ep in self.requests[peer_url] if now - ts < 60]

            # Count requests for this endpoint
            endpoint_requests = [ep for ts, ep in self.requests[peer_url] if ep == endpoint]
...
if __name__ == "__main__":
    ...
    peer_manager, block_sync, require_auth = initialize_secure_p2p(
        db_path="/tmp/test_chain.db", local_host="127.0.0.1", local_port=8088
    )
