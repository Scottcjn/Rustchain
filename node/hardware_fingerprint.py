...
class HardwareFingerprint:
    ...
    def collect_cache_timing(iterations: int = CACHE_TIMING_ITERATIONS) -> Dict:
        ...
        for i in range(len(keys) - 1):
            if "error" not in latencies[keys[i]] and "error" not in latencies[keys[i + 1]]:
                ratio = (
                    latencies[keys[i + 1]]["random_ns"] / latencies[keys[i]]["random_ns"]
                    if latencies[keys[i]]["random_ns"] > 0
                    else 0
                )
    ...
    def check_anti_emulation() -> Dict:
        ...
        jitter_cv = (
            statistics.stdev(jitter_test) / statistics.mean(jitter_test) if statistics.mean(jitter_test) > 0 else 0
        )
