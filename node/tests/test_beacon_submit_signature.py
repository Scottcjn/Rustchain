...
def _build_valid_envelope(
    agent_id: str = None, kind: str = "heartbeat", payload: dict = None, signing_key: SigningKey = None
):
...
class TestBeaconSubmitValidPayloads(unittest.TestCase):
    ...
    def test_accepts_valid_heartbeat_envelope(self):
        ...
        try:
            ...
            with sqlite3.connect(db_path) as conn:
                row = conn.execute(
                    "SELECT agent_id, kind, nonce, sig FROM beacon_envelopes WHERE id = ?", (result["id"],)
                ).fetchone()
    ...
    def test_accepts_valid_want_envelope(self):
        ...
        try:
            beacon_anchor.init_beacon_table(db_path)
            envelope, _ = _build_valid_envelope(kind="want", payload={"request": "compute_task", "budget": 100})
    ...
    def test_accepts_valid_bounty_envelope(self):
        ...
        try:
            beacon_anchor.init_beacon_table(db_path)
            envelope, _ = _build_valid_envelope(kind="bounty", payload={"issue": "#123", "reward": "50 RTC"})
    ...
    def test_accepts_valid_mayday_envelope(self):
        ...
        try:
            beacon_anchor.init_beacon_table(db_path)
            envelope, _ = _build_valid_envelope(kind="mayday", payload={"emergency": "system_failure"})
    ...
    def test_accepts_valid_accord_envelope(self):
        ...
        try:
            beacon_anchor.init_beacon_table(db_path)
            envelope, _ = _build_valid_envelope(kind="accord", payload={"contract_id": "ctr_001", "terms": "accepted"})
    ...
    def test_accepts_valid_pushback_envelope(self):
        ...
        try:
            beacon_anchor.init_beacon_table(db_path)
            envelope, _ = _build_valid_envelope(
                kind="pushback", payload={"dispute": "contract_ctr_001", "reason": "terms_violation"}
            )
    ...
    def test_multiple_valid_envelopes_from_different_agents(self):
        ...
        try:
            ...
            envelopes = []
            for i in range(3):
                env, _ = _build_valid_envelope(kind="heartbeat", payload={"agent_num": i})
