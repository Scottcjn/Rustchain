...
def test_save_block_serializes_with_template_production(tmp_path, monkeypatch):
    ...
    latest = {"height": 0, "block_hash": "old-head"}
    producer.get_latest_block = lambda: latest

    def save_under_lock(block):
        pool.confirm_entered.set()
        assert pool.release_confirm.wait(2)
        latest["height"] = block.height
        latest["block_hash"] = block.hash
        return True

    producer._save_block_unlocked = save_under_lock
