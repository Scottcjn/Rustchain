...
class TestBridgeVoidSecurity(unittest.TestCase):
    ...
    def test_void_only_releases_bridge_locks(self):
        ...
        self.conn.execute("INSERT INTO lock_ledger (bridge_transfer_id, lock_type, status) VALUES (?, 'bridge_transfer', 'locked')", (transfer_id,))
        ...
        bridge_lock = cursor.execute("SELECT status FROM lock_ledger WHERE lock_type = 'bridge_transfer'").fetchone()
