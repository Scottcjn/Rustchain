...
from arch_cross_validation import ARCHITECTURE_PROFILES, normalize_arch, validate_arch_consistency
...
def test_g4_x86_spoofing():
    fp = {
        "checks": {
            "simd_identity": {
                "passed": True,
                "data": {"has_sse2": True, "has_avx2": True, "has_altivec": False, "simd_type": "sse_avx"},
            },
...
def test_frozen_profile():
    ...
    score, details = validate_arch_consistency(fp, "modern_x86")
    assert details["scores"]["clock_consistency"] == 0.3, (
        f"frozen clock should score 0.3, got {details['scores']['clock_consistency']}"
    )
...
def test_cpu_brand_consistency():
    ...
    assert score_good > score_bad, "G4 with Intel brand should score lower"
...
def test_score_interpretation_levels():
    for arch in ["g4", "modern_x86"]:
        fp = {
            "checks": {
                "simd_identity": {"passed": True, "data": {"has_sse2": True}},
                "clock_drift": {"passed": True, "data": {"cv": 0.002, "samples": 200}},
                "cache_timing": {"passed": True, "data": {"tone_ratios": [1.5]}},
            }
        }
