...
def select_active_fingerprint_checks(
    previous_epoch_block_hash: str, active_count: int = ACTIVE_FINGERPRINT_CHECK_COUNT
) -> Tuple[str, ...]:
...
def calculate_epoch_rewards_time_aged(
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        try:
            cursor.execute("SELECT miner_pk, weight FROM epoch_enroll WHERE epoch = ?", (epoch,))
    ...
    if total_weight <= 0:
        logger.warning(
            "Epoch %d: total_weight=0 — all %d miners have zero weight, no rewards distributed",
            epoch,
            len(weighted_miners),
        )
        return {}
