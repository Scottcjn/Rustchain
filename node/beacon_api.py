...
def beacon_join():
    ...
    try:
        ...
        db = get_db()
        existing = db.execute("SELECT pubkey_hex FROM relay_agents WHERE agent_id = ?", (agent_id,)).fetchone()
...
def get_contracts():
    ...
    try:
        db = get_db()
        rows = db.execute("SELECT * FROM beacon_contracts ORDER BY created_at DESC").fetchall()
...
def get_bounties():
    ...
    try:
        db = get_db()
        rows = db.execute("SELECT * FROM beacon_bounties WHERE state = 'open' ORDER BY reward_rtc DESC").fetchall()
...
def sync_bounties():
    ...
    try:
        ...
        if os.environ.get("RC_DISABLE_SSL_VERIFY", "0") == "1":
            ...
            logging.warning(
                "[beacon_api] SSL verification disabled via RC_DISABLE_SSL_VERIFY — not recommended for production"
            )
...
def complete_bounty(bounty_id):
    ...
    try:
        ...
        bounty = db.execute("SELECT state, claimant_agent FROM beacon_bounties WHERE id = ?", (bounty_id,)).fetchone()
        if not bounty:
            return jsonify({"error": "Bounty not found"}), 404
