...
def process_claims_batch(
    ...
    try:
        ...
        # Unrecoverable error (wallet crash, client exception) — release pool
        # funds and mark claims as failed since we can't determine whether the
        # transaction was partially broadcast.
    ...
    if not success:
        # Transient broadcast failure — release pool funds AND release
        # reserved claims back to 'approved' so they can be retried by
        # a future settlement worker.  Do NOT call update_claims_failed()
        # here: that moves status to 'failed', which would make the claims
        # permanently unretryable (the bug identified by surim0n).
        release_rewards_pool_funds(db_path, required_amount)
        released_count = release_reserved_claims_for_settlement(
            db_path,
            [c["claim_id"] for c in claims_to_process],
            batch_id,
            error or "Transaction broadcast failed",
        )
        result["released_count"] = released_count
        result["failed_count"] = 0  # Not failed — released for retry
