"""
RIP-302 Auto-Matching Engine
==============================
Scores and ranks workers for job matching based on reputation,
category expertise, and completion rate.

Endpoints:
  GET /agent/match/<job_id>         — Ranked worker suggestions for a job
  GET /agent/match/suggest           — Best-fit jobs for a worker wallet
...
CATEGORY_WEIGHTS = {
    "research":    1.0,
    "code":        1.2,   # Higher weight — harder to fake competence
    "video":       0.9,
    "audio":       0.9,
    "writing":     0.85,
    "translation": 0.85,
...
MIN_JOBS_FOR_CATEGORY_SCORE = 3    # Need ≥3 category jobs before trusting category expertise
...
def init_auto_match_tables(db_path: str):
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        # Match suggestions cache (rate-limited per job)
        c.execute("""
...
def register_auto_match(app: Flask, db_path: str):
            ...
            if total and total > 0:
                avg_r = c.execute("""
                    ...
                    WHERE ratee_wallet = ? AND role = 'poster_rates_worker'
                      AND created_at > ?
                """, (wallet_id, cutoff)).fetchone()[0] or 0.0

                c.execute("""
                    INSERT INTO agent_category_stats
                    (wallet_id, category, jobs_in_cat, completed, disputed,
                     expired, total_earned, avg_rating, updated_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                    ON CONFLICT(wallet_id, category) DO UPDATE SET
                        jobs_in_cat = ?, completed = ?, disputed = ?,
                        expired = ?, total_earned = ?, avg_rating = ?, updated_at = ?
                """, (wallet_id, cat, total, completed, disputed,
                      expired, earned, avg_r, now,
                      total, completed, disputed, expired, earned, avg_r, now))

    # -----------------------------------------------------------------------
    # Helper: compute match score for a worker on a job
    # -----------------------------------------------------------------------
    def _compute_match_score(c: sqlite3.Cursor, wallet_id: str, job_category: str,
                             job_reward: float) -> dict:
        """
        ...
        if not rep:
            rep_cols = ["wallet_id","jobs_posted","jobs_completed_as_poster",
                        "jobs_completed_as_worker","jobs_disputed","jobs_expired",
                        "total_rtc_paid","total_rtc_earned","avg_rating",
                        "rating_count","first_seen","last_active"]
            rep = dict(zip(rep_cols,
                           [wallet_id,0,0,0,0,0,0,0,0,0,None,None]))
        ...
        if cat_row and cat_row["jobs_in_cat"] >= MIN_JOBS_FOR_CATEGORY_SCORE:
            cat_total = cat_row["jobs_in_cat"]
            cat_completed = cat_row["completed"]
    ...
    def agent_match_job(job_id: str):
        ...
        try:
            ...
            potential_workers = c.execute("""
                ...
                FROM agent_reputation aw
                LEFT JOIN agent_job_views ajv ON ajv.job_id = ? AND ajv.wallet_id = aw.wallet_id
                LEFT JOIN agent_jobs ajc
                    ON ajc.worker_wallet = aw.wallet_id
                    AND ajc.category = ?
                    AND ajc.status = 'completed'
                WHERE aw.wallet_id != ?
                  AND aw.wallet_id != ?
                  AND (
                      ajv.wallet_id IS NOT NULL
                      OR ajc.worker_wallet IS NOT NULL
                  )
                  AND aw.last_active > ?
            """, (job_id, job_category, j["poster_wallet"],
                  j.get("worker_wallet", ""), now - RECENCY_WINDOW_SECS
            )).fetchall()

            # If no views and no category workers, use globally active workers
            if not potential_workers:
                potential_workers = c.execute("""
                    SELECT DISTINCT worker_wallet AS wallet_id
                    FROM agent_jobs
    ...
    def agent_match_view(job_id: str):
        """Record that a worker viewed a job (helps match quality)."""
    ...
    def agent_match_suggest():
        ...
        try:
            ...
            for row in open_jobs:
                ...
                for k in ["wallet_id", "category", "total_score"]:
                    pass  # keep all keys
                scored.append(score_info)
            ...
            if category:
                ...
                rows = c.execute("""
                    SELECT acs.*,
                           ...
                           COALESCE(ar.jobs_completed_as_worker, 0) AS total_completed
                ...
                rows = c.execute("""
                    ...
                    LEFT JOIN agent_category_stats acs ON acs.wallet_id = ar.wallet_id
                    GROUP BY ar.wallet_id
                    HAVING ar.last_active > ?
                    ORDER BY ar.jobs_completed_as_worker DESC,
                             ar.avg_rating DESC
                    LIMIT ?
                ...
                for rank, row in enumerate(rows, 1):
                    r = dict(row)
                    total = (r["jobs_completed_as_worker"] +
                             r["jobs_disputed"] +
                             r["jobs_expired"])
                    success_rate = r["jobs_completed_as_worker"] / total if total > 0 else 0
                    ...
                    results.append({
                        ...
                        "avg_rating": round(r["avg_rating"] or 0, 2),
                        "total_rtc_earned": round(r["total_rtc_earned"], 4),
    ...
    # -----------------------------------------------------------------------
    # GET /agent/match/stats — Auto-match engine statistics
    # -----------------------------------------------------------------------
    ...
    def agent_match_stats():
        ...
        try:
            ...
            stats["cache_freshness"] = c.execute("""
                SELECT COUNT(*), AVG(? - cached_at) FROM agent_match_cache
            """, (now,)).fetchone()[0]
