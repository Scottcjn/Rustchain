...
class TestUtxoDB(unittest.TestCase):
    ...
    def _apply_coinbase(self, address: str, value_nrtc: int, block_height: int = 1) -> bool:
    ...
    def test_mempool_add_and_remove(self):
        ...
        self.assertTrue(self.db.mempool_check_double_spend(boxes[0]["box_id"]))
    ...
    def test_spending_proof_accepted_without_verification(self):
        ...
        ok = self.db.apply_transaction(
            {
                "tx_type": "transfer",
                "inputs": [{"box_id": boxes[0]["box_id"], "spending_proof": "TOTALLY_BOGUS"}],
                "outputs": [{"address": "bob", "value_nrtc": 100 * UNIT}],
                "fee_nrtc": 0,
            },
            block_height=10,
        )
        self.assertTrue(ok, "UTXO layer should accept any spending_proof (verification is endpoint's job)")
        ...
        ok = self.db.apply_transaction(
            {
                "tx_type": "mining_reward",
                "inputs": [],
                "outputs": [{"address": "attacker", "value_nrtc": MAX_COINBASE_OUTPUT_NRTC + 1}],
                "fee_nrtc": 0,
                "timestamp": int(time.time()),
            },
            block_height=10,
...
class TestCoinSelect(unittest.TestCase):
    ...
    def test_smallest_first(self):
        utxos = [self._box(50 * UNIT), self._box(10 * UNIT), self._box(30 * UNIT)]
...
class TestMultiInputTransfer(unittest.TestCase):
    ...
    def test_consolidation(self):
        ...
        ok = self.db.apply_transaction(
            {
                "tx_type": "transfer",
                "inputs": [{"box_id": b["box_id"], "spending_proof": "sig"} for b in boxes],
                "outputs": [
    ...
    def test_transaction_recorded(self):
        ...
        conn = self.db._conn()
        try:
            row = conn.execute("SELECT COUNT(*) AS n FROM utxo_transactions").fetchone()
            self.assertEqual(row["n"], 1)

            tx = conn.execute("SELECT * FROM utxo_transactions").fetchone()
