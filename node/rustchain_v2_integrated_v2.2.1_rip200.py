...
def ensure_epoch_enroll_integer_weights(conn: sqlite3.Connection):
    ...
    rows = conn.execute("SELECT epoch, miner_pk, weight FROM epoch_enroll").fetchall()  # fetchall-ok: bounded-by-schema
...
def _table_columns(conn: sqlite3.Connection, table_name: str) -> set:
    return {row[1] for row in conn.execute(f"PRAGMA table_info({table_name})").fetchall()}  # fetchall-ok: pragma-result
...
def admin_wallet_review_holds():
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        rows = conn.execute(sql, params).fetchall()  # fetchall-ok: bounded-by-schema
    return jsonify({
        "ok": True,
        "count": len(rows),
        "entries": [
...
def withdrawal_history(miner_pk):
    ...
    with sqlite3.connect(DB_PATH) as c:
        rows = c.execute("""
            ...
            ORDER BY created_at DESC
            LIMIT ?
        """, (miner_pk, limit)).fetchall()  # fetchall-ok: bounded-by-schema
...
def genesis_export():
    ...
    with _db() as db:
        ...
        act = db.execute("""SELECT signer_id, pubkey_hex FROM gov_signers
                            WHERE active=1 ORDER BY signer_id""").fetchall()  # fetchall-ok: bounded-by-schema
...
def get_balance(miner_pk):
    ...
    with sqlite3.connect(DB_PATH) as c:
        cur = c.cursor()
        cols = {r[1] for r in cur.execute("PRAGMA table_info(balances)").fetchall()}  # fetchall-ok: pragma-result
...
def api_miners():
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        rows = c.execute("""
            ...
            LIMIT ? OFFSET ?
        """, (now - 3600, limit, offset)).fetchall()  # fetchall-ok: bounded-by-schema
        ...
        with sqlite3.connect(DB_PATH) as c:
            ...
            if has_hist:
                ...
                rows = c.execute("""
                    ...
                    GROUP BY bucket
                    ORDER BY bucket ASC
                """, (miner_id, start)).fetchall()  # fetchall-ok: bounded-by-schema
...
def api_balances():
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        cols = set()
        try:
            for r in c.execute("PRAGMA table_info(balances)").fetchall():  # fetchall-ok: pragma-result
                cols.add(str(r["name"]))
        except Exception:
            cols = set()
...
def list_oui_deny():
    ...
    if not is_admin(request):
        return jsonify({"ok": False, "error": "forbidden"}), 403
    with sqlite3.connect(DB_PATH) as conn:
        rows = conn.execute("SELECT oui, vendor, added_ts, enforce FROM oui_deny ORDER BY vendor").fetchall()  # fetchall-ok: bounded-by-schema
    return jsonify({
        "ok": True,
        "count": len(rows),
...
def confirm_pending():
    ...
    try:
        ...
        ready = c.execute("""
            ...
            ORDER BY id ASC
        """, (now,)).fetchall()  # fetchall-ok: bounded-by-schema
...
def api_wallet_balances_all():
    ...
    with sqlite3.connect(DB_PATH) as db:
        rows = fetch_page(
            ...
            "SELECT miner_id, amount_i64 FROM balances ORDER BY amount_i64 DESC",
            (),
            limit=1000  # Enforcing max bound for safety
        )
...
def _balance_columns(c: sqlite3.Cursor) -> set:
    return {row[1] for row in c.execute("PRAGMA table_info(balances)").fetchall()}  # fetchall-ok: pragma-result
