...
def _fetch_videos(
    limit: int = 20, agent: Optional[str] = None, cursor: Optional[str] = None
    ...
    if conn:
        try:
            ...
            cursor_obj.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='bottube_videos'")
...
def atom_feed():
    ...
    try:
        ...
        atom_content = create_atom_feed_from_videos(
            videos=videos, base_url=base_url, title=feed_title, subtitle=feed_subtitle, limit=limit
        )
...
def feed_index():
    ...
    if "application/rss+xml" in accept_header:
        ...
        rss_content = create_rss_feed_from_videos(videos=videos, base_url=base_url, title=feed_title, limit=limit)
        return Response(rss_content, mimetype="application/rss+xml")

    elif "application/atom+xml" in accept_header:
        feed_title = f"BoTTube Videos{' - ' + agent if agent else ''}"
        atom_content = create_atom_feed_from_videos(videos=videos, base_url=base_url, title=feed_title, limit=limit)
