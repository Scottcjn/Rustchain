...
class WebSocketFeed:
    ...
    def _register_events(self):
        ...
        def handle_connect():
            ...
            emit("connection_status", {"status": "connected", "server_version": "2.2.1-ws", "ping_interval": 25})
        ...
        def handle_ping():
            ...
            emit("pong", {"timestamp": time.time(), "server_time": datetime.utcnow().isoformat()})
    ...
    def set_fetch_callbacks(self, fetch_blocks=None, fetch_miners=None, fetch_epoch=None, fetch_health=None):
    ...
    def run(self, host: str = "0.0.0.0", port: int = None, **kwargs):  # nosec B104
...
def broadcast_block(height: int, hash: str, timestamp: float, miners_count: int, reward: float, epoch: int, slot: int):
    """Broadcast a new block event"""
    block = BlockEvent(
        height=height, hash=hash, timestamp=timestamp, miners_count=miners_count, reward=reward, epoch=epoch, slot=slot
    )
    ws_feed.broadcast_block(block)


def broadcast_attestation(
    miner_id: str, device_arch: str, multiplier: float, epoch: int, weight: float, ticket_id: str
):
...
def broadcast_epoch_settlement(epoch: int, total_blocks: int, total_reward: float, miners_count: int):
