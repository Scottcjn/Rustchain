...
```bash
curl -fsS https://rustchain.org/api/miners | jq '.[].miner'
```
...
```bash
curl -fsS https://rustchain.org/api/miners | jq '.[] | select(.miner=="YOUR_MINER_ID")'
```
...
3. Check network connectivity: `curl -fsS https://rustchain.org/health`
