...
class TestInitDb:
    ...
    def test_idempotent(self, app):
        ...
        cr.init_db()  # first call in fixture
        cr.init_db()  # second call — should succeed silently
...
class TestRegisterRoute:
    ...
    def test_register_missing_fields(self, client):
        """POST /register without required fields should return 400."""
        response = client.post("/register", data={}, follow_redirects=False)
        # Flask raises BadRequestKeyError or 400 depending on config
        assert response.status_code in (400, 500) or response.status_code == 302
...
class TestApiContributors:
    ...
    def test_api_contributor_fields(self, client, seed_contributor):
        ...
        assert "github_username" in contrib
        assert "type" in contrib
        assert "wallet" in contrib
        assert "registered" in contrib
...
class TestApproveRoute:
    def test_approve_pending_contributor(self, client):
        ...
        # Register a pending contributor first
