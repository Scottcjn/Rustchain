...
class TestOEmbedEndpoint(unittest.TestCase):
    ...
    def test_oembed_accepts_current_host_url(self):
        """Test oEmbed accepts URLs generated by the current app host."""
        response = self.client.get("/oembed?url=http://localhost/watch/demo-001")
        data = response.get_json()
        self.assertEqual(response.status_code, 200)
