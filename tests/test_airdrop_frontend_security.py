...
def test_airdrop_defines_safe_render_helpers():
    html = source()

    assert "function escapeHtml(value)" in html
    assert "function safeNumber(value, fallback = 0)" in html
    assert "function shortWallet(value, head = 6, tail = 4)" in html
    assert "d.textContent = String(value ?? '');" in html


def test_airdrop_escapes_oauth_wallet_and_claim_fields_before_inner_html():
    html = source()
