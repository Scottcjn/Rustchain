...
def test_buy_order_defers_htlc_secret_to_matching_seller(tmp_path):
    ...
    with module.app.test_client() as client:
        ...
        with patch.object(module.requests, "post") as mock_post:
            mock_response = MagicMock(ok=True, text='{"ok": true}')
            mock_response.json.return_value = {"ok": True, "details": {"phase": "pending", "pending_id": "test_pending_id", "tx_hash": "test_tx_hash"}}
