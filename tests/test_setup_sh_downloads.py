...
def test_setup_sh_uses_platform_specific_miner_paths():
    ...
    assert 'RC_CRYPTO_PATH="miners/linux/miner_crypto.py"' in script
    ...
    assert 'RC_CRYPTO_PATH=""' in script
    assert 'RC_MINER_URL="${RC_BASE_URL}/${RC_MINER_PATH}"' in script
    assert 'RC_FP_URL="${RC_BASE_URL}/${RC_FP_PATH}"' in script
    assert 'RC_CRYPTO_URL="${RC_BASE_URL}/${RC_CRYPTO_PATH}"' in script
    assert 'import requests, nacl' in script
    assert 'pip install requests PyNaCl' in script
...
def test_setup_sh_fails_fast_when_downloads_fail():
    ...
    assert 'download_file "$RC_CRYPTO_URL" "$INSTALL_DIR/miner_crypto.py"' in script
